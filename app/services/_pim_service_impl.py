...
from app.services.variant_metadata_service import build_medusa_variant_metadata, technical_attributes_metadata
from app.services.customs_service import (
    VARIANT_CUSTOMS_FIELDS,
    _apply_variant_customs_payload,
    _serialize_variant_customs_fields,
    delete_variant_customs_additional_code,
    upsert_variant_customs_additional_code,
)
from app.services.pricing_service import (
    _serialize_price_tier,
    _sorted_price_tiers,
    _sync_base_tier_from_variant,
    calculated_tier_margin_metrics,
...
from app.services.technical_attribute_service import (
    _serialize_variant_technical_attribute,
...
PRODUCT_STATUS_OPTIONS = ["draft", "active", "inactive"]
VARIANT_STATUS_OPTIONS = ["draft", "active", "inactive"]
...
SKU_START_NUMBER = 1000
SKU_VARIANT_LIMIT = 99
...
def _sku_product_number(value: str | None) -> int | None:
    raw = (value or "").strip()
    if not raw:
        return None
    product_number = raw.split(".", 1)[0]
    if not product_number.isdigit():
        return None
    return int(product_number)


def next_product_sku(session: Session) -> str:
    max_number = SKU_START_NUMBER - 1
    for value in session.scalars(select(Product.sku)):
        number = _sku_product_number(value)
        if number is not None and number >= SKU_START_NUMBER:
            max_number = max(max_number, number)
    return str(max_number + 1)


def next_variant_sku(session: Session, product: Product) -> str:
    product_sku = str(product.sku or "").strip()
    if not product_sku:
        raise ValueError("Produkt-SKU fehlt")
    prefix = f"{product_sku}."
    max_variant_number = 0
    existing_skus = set(session.scalars(select(ProductVariant.sku).where(ProductVariant.product_id == product.id)))
    for sku in existing_skus:
        ...
        if suffix.isdigit():
            max_variant_number = max(max_variant_number, int(suffix))
    next_number = max_variant_number + 1
    if next_number > SKU_VARIANT_LIMIT:
        raise ValueError("Mehr als 99 Varianten erfordern eine Erweiterung des SKU-Konzepts")
    candidate = f"{product_sku}.{next_number:02d}"
    if candidate in existing_skus:
        raise ValueError(f"Varianten-SKU {candidate} ist bereits vergeben")
    return candidate
...
def _normalize_product_status(value: str | None, default: str = "draft") -> str:
    normalized = (value or default).strip().lower()
    if normalized in PRODUCT_STATUS_OPTIONS:
        return normalized
    if normalized in {"ready", "published", "imported"}:
        return "active"
    if normalized in {"archived", "archiviert", "inaktiv"}:
        return "inactive"
    return default
        ...
        rows.append(
            {
                ...
                "photo_filter": "mit Photo" if primary_asset else "ohne Photo",
...
def upsert_product_with_variant(
    ...
    **chemical_fields: object,
    ...
    if not product_sku:
        product_sku = next_product_sku(session)
    product = None
    if product is None:
        product = session.scalar(select(Product).where(Product.sku == product_sku))
    handle = _unique_handle(session, title or product_sku, product_id=product.id if product else None)
    if product is None:
        product = Product(
            sku=product_sku,
            handle=handle,
            source_language=source_language or "en",
            title=title or product_sku,
            ...
            brand=brand,
            status=_normalize_product_status(status, default="active"),
        ...
        product.title = title or product_sku
        ...
        product.brand = brand
        product.status = _normalize_product_status(status, default=product.status or "active")
    ...
    normalized_variant_sku = (variant_sku or "").strip()
    if not normalized_variant_sku:
        normalized_variant_sku = next_variant_sku(session, product)
    variant = session.scalar(select(ProductVariant).where(ProductVariant.sku == normalized_variant_sku))
    if variant is None:
        variant = ProductVariant(
            product=product,
            sku=normalized_variant_sku,
            manufacturer_sku=(manufacturer_sku or "").strip() or None,
        ...
        if manufacturer_sku is not None:
            variant.manufacturer_sku = manufacturer_sku.strip() or None
        if getattr(variant, "vendor_description", None) is None:
            variant.vendor_description = None
        ...
        if sales_unit:
            variant.sales_unit = sales_unit
        if pack_quantity is not None:
            variant.pack_quantity = pack_quantity
        if pack_unit:
            variant.pack_unit = pack_unit
        if packaging:
...
def create_product(session: Session, payload: ProductCreate, variant_payload: VariantCreate | None = None) -> tuple[Product, ProductVariant | None]:
    product_sku = (payload.sku or "").strip() or next_product_sku(session)
    brand = get_or_create_brand(session, payload.brand_name)
    handle = _unique_handle(session, payload.handle or payload.title or product_sku)
    product = Product(
        sku=product_sku,
    ...
    if variant_payload is not None:
        variant_sku = (variant_payload.sku or "").strip() or next_variant_sku(session, product)
        variant = ProductVariant(
            product_id=product.id,
            sku=variant_sku,
            manufacturer_sku=(variant_payload.manufacturer_sku or "").strip() or None,
            vendor_description=(variant_payload.vendor_description or "").strip() or None,
            ...
            sales_unit=variant_payload.sales_unit,
            pack_quantity=variant_payload.pack_quantity,
            pack_unit=variant_payload.pack_unit,
            ...
            status=_normalize_variant_status(variant_payload.status),
        )
        _apply_variant_customs_payload(variant, variant_payload)
        session.add(variant)
        session.flush()
    return product, variant
...
def update_product(session: Session, product_id: int, payload: ProductUpdate) -> Product:
    ...
    product.status = _normalize_product_status(payload.status, default=product.status or "draft")
...
def update_variant(session: Session, variant_id: int, payload: VariantUpdate) -> ProductVariant:
    ...
    if payload.sku is not None:
        normalized_sku = payload.sku.strip()
        if not normalized_sku:
            raise ValueError("Varianten-SKU darf nicht leer sein")
        existing = session.scalar(select(ProductVariant).where(ProductVariant.sku == normalized_sku, ProductVariant.id != variant_id))
        if existing is not None:
            raise ValueError("Varianten-SKU ist bereits vergeben")
        variant.sku = normalized_sku
    if payload.manufacturer_sku is not None:
        variant.manufacturer_sku = payload.manufacturer_sku.strip() or None
    if payload.vendor_description is not None:
        variant.vendor_description = payload.vendor_description.strip() or None
    ...
    variant.sales_unit = payload.sales_unit
    variant.pack_quantity = payload.pack_quantity
    variant.pack_unit = payload.pack_unit
    ...
    _apply_variant_customs_payload(variant, payload)
    if payload.status:
...
def hard_delete_product(session: Session, product_id: int, confirmation: str | None = None) -> dict[str, int | str]:
    product = session.get(Product, int(product_id))
    if product is None:
        raise ValueError("Produkt nicht gefunden")
    expected_tokens = {str(product.id), str(product.sku)}
    if (confirmation or "").strip() not in expected_tokens:
        raise ValueError("Bestätigung muss Produkt-ID oder Produkt-SKU exakt enthalten")

    variant_ids = list(session.scalars(select(ProductVariant.id).where(ProductVariant.product_id == product.id)))
    asset_filter = Asset.product_id == product.id
    if variant_ids:
        asset_filter = asset_filter | Asset.variant_id.in_(variant_ids)
    asset_ids = list(session.scalars(select(Asset.id).where(asset_filter)))
    product_translation_ids = list(session.scalars(select(ProductTranslation.id).where(ProductTranslation.product_id == product.id)))
    variant_translation_ids = list(
        session.scalars(select(VariantTranslation.id).where(VariantTranslation.variant_id.in_(variant_ids)))
        if variant_ids
        else []
    )
    price_tier_ids = list(
        session.scalars(select(ProductVariantPriceTier.id).where(ProductVariantPriceTier.variant_id.in_(variant_ids)))
        if variant_ids
        else []
    )

    product_sdb_ids = list(session.scalars(select(ProductSDB.id).where(ProductSDB.product_id == product.id)))
    suva_check_ids = list(session.scalars(select(ProductSuvaCheck.id).where(ProductSuvaCheck.product_id == product.id)))
    duplicate_group_ids = set(session.scalars(select(ProductDuplicateGroup.id).where(ProductDuplicateGroup.master_product_id == product.id)))
    duplicate_group_ids.update(session.scalars(select(ProductDuplicateGroupItem.group_id).where(ProductDuplicateGroupItem.product_id == product.id)))

    if duplicate_group_ids:
        session.execute(delete(ProductMergePreview).where(ProductMergePreview.group_id.in_(duplicate_group_ids)))
        session.execute(delete(ProductDuplicateGroupItem).where(ProductDuplicateGroupItem.group_id.in_(duplicate_group_ids)))
        session.execute(delete(ProductDuplicateGroup).where(ProductDuplicateGroup.id.in_(duplicate_group_ids)))
    if suva_check_ids:
        session.execute(delete(ProductSuvaCheckItem).where(ProductSuvaCheckItem.check_id.in_(suva_check_ids)))
        session.execute(delete(ProductSuvaCheck).where(ProductSuvaCheck.id.in_(suva_check_ids)))
    if product_sdb_ids:
        session.execute(delete(ProductSDBLLMRun).where(ProductSDBLLMRun.product_sdb_id.in_(product_sdb_ids)))
        session.execute(delete(ProductSDB).where(ProductSDB.id.in_(product_sdb_ids)))

    session.execute(delete(SDSReviewIssue).where(SDSReviewIssue.product_id == product.id))
    session.execute(delete(ChemicalDocument).where(ChemicalDocument.product_id == product.id))
    session.execute(delete(ProductChemicalEnrichment).where(ProductChemicalEnrichment.product_id == product.id))
    session.execute(delete(ProductEnrichmentLog).where(ProductEnrichmentLog.product_id == product.id))
    session.execute(delete(ProductEnrichmentCandidate).where(ProductEnrichmentCandidate.product_id == product.id))
    session.execute(delete(ProductAssetCandidate).where(ProductAssetCandidate.product_id == product.id))
    session.execute(delete(ProductMergeLog).where(ProductMergeLog.master_product_id == product.id))

    if variant_ids:
        session.execute(delete(VariantChannelListing).where(VariantChannelListing.variant_id.in_(variant_ids)))
        session.execute(delete(VariantCategoryMapping).where(VariantCategoryMapping.variant_id.in_(variant_ids)))
        session.execute(delete(VariantTranslation).where(VariantTranslation.variant_id.in_(variant_ids)))
        session.execute(delete(ProductVariantPriceTier).where(ProductVariantPriceTier.variant_id.in_(variant_ids)))
        technical_attribute_ids = list(session.scalars(select(VariantTechnicalAttribute.id).where(VariantTechnicalAttribute.variant_id.in_(variant_ids))))
        if technical_attribute_ids:
            session.execute(
                delete(VariantTechnicalAttributeValueTranslation).where(
                    VariantTechnicalAttributeValueTranslation.technical_attribute_id.in_(technical_attribute_ids)
                )
            )
        session.execute(delete(VariantTechnicalAttribute).where(VariantTechnicalAttribute.variant_id.in_(variant_ids)))
        session.execute(
            delete(MedusaSyncMapping).where(
                (MedusaSyncMapping.entity_type.in_(["variant", "price"]))
                & (MedusaSyncMapping.local_entity_id.in_(variant_ids))
            )
        )
        session.execute(
            delete(MedusaSyncRunItem).where(
                (MedusaSyncRunItem.entity_type.in_(["variant", "price"]))
                & (MedusaSyncRunItem.local_entity_id.in_(variant_ids))
            )
        )

    session.execute(delete(ProductChannelListing).where(ProductChannelListing.product_id == product.id))
    session.execute(delete(ProductCategoryMapping).where(ProductCategoryMapping.product_id == product.id))
    session.execute(delete(ProductCategoryAssignment).where(ProductCategoryAssignment.product_id == product.id))
    session.execute(delete(ProductTranslation).where(ProductTranslation.product_id == product.id))
    session.execute(
        delete(MedusaSyncMapping).where(
            (MedusaSyncMapping.entity_type == "product")
            & (MedusaSyncMapping.local_entity_id == product.id)
        )
    )
    session.execute(
        delete(MedusaSyncRunItem).where(
            (MedusaSyncRunItem.entity_type == "product")
            & (MedusaSyncRunItem.local_entity_id == product.id)
        )
    )
    ...
    if variant_ids:
        session.execute(delete(ProductVariant).where(ProductVariant.id.in_(variant_ids)))
    ...
    return {
        ...
        "assets_deleted": len(asset_ids),
        "translations_deleted": len(product_translation_ids) + len(variant_translation_ids),
        "price_tiers_deleted": len(price_tier_ids),
    }


def hard_delete_variant(session: Session, variant_id: int) -> dict[str, int | str]:
    variant = session.get(ProductVariant, int(variant_id))
    if variant is None:
        raise ValueError("Variante nicht gefunden")
    asset_ids = list(session.scalars(select(Asset.id).where(Asset.variant_id == variant.id)))
    translation_count = session.scalar(select(func.count()).select_from(VariantTranslation).where(VariantTranslation.variant_id == variant.id)) or 0
    price_tier_count = session.scalar(select(func.count()).select_from(ProductVariantPriceTier).where(ProductVariantPriceTier.variant_id == variant.id)) or 0

    session.execute(delete(VariantChannelListing).where(VariantChannelListing.variant_id == variant.id))
    session.execute(delete(VariantCategoryMapping).where(VariantCategoryMapping.variant_id == variant.id))
    session.execute(delete(VariantTranslation).where(VariantTranslation.variant_id == variant.id))
    session.execute(delete(ProductVariantPriceTier).where(ProductVariantPriceTier.variant_id == variant.id))
    technical_attribute_ids = list(session.scalars(select(VariantTechnicalAttribute.id).where(VariantTechnicalAttribute.variant_id == variant.id)))
    if technical_attribute_ids:
        session.execute(
            delete(VariantTechnicalAttributeValueTranslation).where(
                VariantTechnicalAttributeValueTranslation.technical_attribute_id.in_(technical_attribute_ids)
    ...
    session.execute(delete(VariantTechnicalAttribute).where(VariantTechnicalAttribute.variant_id == variant.id))
    session.execute(
        delete(MedusaSyncMapping).where(
            (MedusaSyncMapping.entity_type.in_(["variant", "price"]))
            & (MedusaSyncMapping.local_entity_id == variant.id)
        )
    )
    session.execute(
        delete(MedusaSyncRunItem).where(
            (MedusaSyncRunItem.entity_type.in_(["variant", "price"]))
            & (MedusaSyncRunItem.local_entity_id == variant.id)
        )
    )
    for asset_id in asset_ids:
        if session.get(Asset, asset_id) is not None:
    ...
    return {
        ...
        "assets_deleted": len(asset_ids),
        "translations_deleted": int(translation_count),
        "price_tiers_deleted": int(price_tier_count),
    }


def hard_delete_variants(session: Session, variant_ids: Iterable[int]) -> dict[str, int]:
    deleted = 0
    assets_deleted = 0
    ...
    for raw_id in variant_ids:
        ...
        result = hard_delete_variant(session, variant_id)
        deleted += 1
    ...
    return {
        ...
        "price_tiers_deleted": price_tiers_deleted,
    }
...
def get_product_detail(session: Session, product_id: int, *, include_sdb: bool = True) -> dict | None:
    options = [
        ...
        selectinload(Product.variants).selectinload(ProductVariant.price_tiers).joinedload(ProductVariantPriceTier.price_list).joinedload(PriceList.sales_channel),
        selectinload(Product.variants).selectinload(ProductVariant.customs_additional_codes),
        selectinload(Product.variants).selectinload(ProductVariant.technical_attributes).selectinload(VariantTechnicalAttribute.translations),
        ...
        "variants": [
            {
                ...
                "option_value": variant.option_value,
                "sales_unit": variant.sales_unit,
                "pack_quantity": float(variant.pack_quantity) if variant.pack_quantity is not None else None,
                "pack_unit": variant.pack_unit,
                "packaging": variant.packaging,
                ...
                "barcode": variant.barcode,
                "status": variant.status,
                **_serialize_variant_customs_fields(variant),
                "technical_attributes": [_serialize_variant_technical_attribute(row) for row in variant.technical_attributes],
                "technical_attributes_metadata": technical_attributes_metadata(variant),
                **_serialize_medusa_mapping(medusa_mappings.get(("variant", variant.id), {}), prefix="medusa"),
                "price_tiers": [_serialize_price_tier(variant, tier, session) for tier in _sorted_price_tiers(variant)],
                "channel_listings": [
...
def list_products(session: Session, archive_filter: str = "active") -> list[dict]:
    ...
    primary_assets: dict[int, Asset] = {}
    if product_ids:
        image_assets = session.scalars(
            select(Asset)
            .where(Asset.product_id.in_(product_ids), Asset.mime_type.like("image/%"))
            .order_by(Asset.product_id.asc(), Asset.sort_order.asc(), Asset.id.asc())
        ).all()
        for asset in image_assets:
            if asset.product_id is not None and asset.product_id not in primary_assets:
                primary_assets[asset.product_id] = asset

    rows = []
    for product in products:
        variants = [variant for variant in product.variants if str(variant.status or "").lower() not in {"archived", "archiviert"}]
        primary_asset = primary_assets.get(product.id)
        color_values = sorted({variant.option_value for variant in variants if variant.option_value})
        rows.append(
            {
                "id": product.id,
                "variant_nav": "V",
...
def list_chemical_products(session: Session) -> list[dict]:
    stmt = (
        select(Product)
        .options(joinedload(Product.brand), selectinload(Product.assets))
...
def list_variants(session: Session, archive_filter: str = "active") -> list[dict]:
    stmt = (
        ...
        .options(
            ...
            selectinload(ProductVariant.price_tiers).joinedload(ProductVariantPriceTier.price_list).joinedload(PriceList.sales_channel),
            selectinload(ProductVariant.customs_additional_codes),
            selectinload(ProductVariant.technical_attributes).selectinload(VariantTechnicalAttribute.translations),
        )
...
def _medusa_extra_fields(product: Product, variant: ProductVariant, handle: str | None, rank: int) -> dict[str, object]:
    ...
    extra_fields: dict[str, object] = {
        ...
        "medusa_variant_metadata": build_medusa_variant_metadata(variant),
    }
...
def list_rule_overview(session: Session) -> list[dict]:
    import_job_count = session.scalar(select(func.count()).select_from(ImportJob)) or 0
    variants_with_sale_price = session.scalar(select(func.count()).select_from(ProductVariant).where(ProductVariant.price.is_not(None))) or 0
    variants_with_cost_price = session.scalar(select(func.count()).select_from(ProductVariant).where(ProductVariant.cost_price.is_not(None))) or 0
    variants_with_tiers = session.scalar(select(func.count(func.distinct(ProductVariantPriceTier.variant_id)))) or 0
    negative_margin_count = (
        session.scalar(
            ...
            .where(
                ...
                ProductVariant.cost_price.is_not(None),
                ProductVariant.price < ProductVariant.cost_price,
            )
        )
        or 0
    )
    return [
        ...
        },
        {
            ...
            "name": "Importjobs",
            "scope": "System",
            "details": f"{import_job_count} Jobs protokolliert",
        },
        {
